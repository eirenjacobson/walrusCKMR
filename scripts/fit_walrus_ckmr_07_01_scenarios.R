...
#BASE_CASE <- substr(TEST_CASE, 1, 5))
suffixes <- "Sd20241023"
...
lglk_with_data <- add_data( lglk_walrus,
    simfile = sprintf( 'WalrusSamples_%s_%s.RData', 
...
print( load( sprintf( './simulation/Nfad_RoI_%s_%s.RData', 
    BASE_CASE, suffixes[ TEST_SUFFIX])))
# variable called 'out', which has "real" abund & RoI param values

# Need to calculate this depending on what values were used in simulation
...
ptru <- c(
  ...
  diff_lgt_fjusurv=  logit( S1) - logit( effective_adult_surv), # 0 => same as adult
