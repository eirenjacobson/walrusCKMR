...
subsample <- function(suffix, samplingscenario){
  ...
  if(samplingscenario == 1){ # leave out 2024, 2027, 2028
    
    allsamples <- allsamples[-which(allsamples$SampY %in% c(2024, 2027, 2028) & allsamples$Lethality == "NONLETHAL"),]
    
  } # end s1
  
  if(samplingscenario == 2){ # leave out 2024 and 2028
    
    allsamples <- allsamples[-which(allsamples$SampY %in% c(2024, 2028) & allsamples$Lethality == "NONLETHAL"),]
    
  } # end s2
  
  if(samplingscenario == 3){ # leave out 2024 
    
    allsamples <- allsamples[-which(allsamples$SampY %in% c(2024) & allsamples$Lethality == "NONLETHAL"),]
    
  } # end s3
  
  if(samplingscenario == 4){ # no effort in 2024, 2027, 2028 and 75% effort in 2025 and 2026
    
    s2025 <- allsamples[which(allsamples$SampY == 2025 & allsamples$Lethality == "NONLETHAL"),]
    s2026 <- allsamples[which(allsamples$SampY == 2026 & allsamples$Lethality == "NONLETHAL"),]
    c25 <- round(nrow(s2025)*.75)
    c26 <- round(nrow(s2026)*.75)
    
    allsamples <- rbind.data.frame(
      allsamples[-which(allsamples$SampY %in% 2024:2028 & allsamples$Lethality == "NONLETHAL"),], 
      s2025[1:c25,], s2026[1:c26,])
  } # end s4
  
  if(samplingscenario == 5){ # no effort in 2024 or 2028, 75% effort in 2025, 2026, 2027
    
    s2025 <- allsamples[which(allsamples$SampY == 2025 & allsamples$Lethality == "NONLETHAL"),]
    s2026 <- allsamples[which(allsamples$SampY == 2026 & allsamples$Lethality == "NONLETHAL"),]
    s2027 <- allsamples[which(allsamples$SampY == 2027 & allsamples$Lethality == "NONLETHAL"),]
    c25 <- round(nrow(s2025)*.75)
    c26 <- round(nrow(s2026)*.75)
    c27 <- round(nrow(s2027)*.75)
    
    allsamples <- rbind.data.frame(allsamples[-which(allsamples$SampY %in% 2024:2028 & allsamples$Lethality == "NONLETHAL"),],
                                   s2025[1:c25,], s2026[1:c26,], s2027[1:c27,])
  } # end s5
  
  if(samplingscenario == 6){ # no effort in 2024, 75% effort in 2025:2028
    
    s2025 <- allsamples[which(allsamples$SampY == 2025 & allsamples$Lethality == "NONLETHAL"),]
    s2026 <- allsamples[which(allsamples$SampY == 2026 & allsamples$Lethality == "NONLETHAL"),]
    s2027 <- allsamples[which(allsamples$SampY == 2027 & allsamples$Lethality == "NONLETHAL"),]
    s2028 <- allsamples[which(allsamples$SampY == 2028 & allsamples$Lethality == "NONLETHAL"),]
    c25 <- round(nrow(s2025)*.75)
    c26 <- round(nrow(s2026)*.75)
    c27 <- round(nrow(s2027)*.75)
  ...
  if(samplingscenario == 7){ # no effort 2026:2028
    
    allsamples <- allsamples[-which(allsamples$SampY %in% 2026:2028 & allsamples$Lethality == "NONLETHAL"),]
